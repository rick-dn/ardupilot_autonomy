...
class ScanSequence(Node):
    def __init__(self):
        ...
        # ========================================
        self.declare_parameter('scan_radius', 10.0)  # meters
        # self.declare_parameter('scan_altitude', 5.0)  # meters above home
        self.declare_parameter('waypoint_spacing', 2.5)  # meters between waypoints
        self.declare_parameter('waypoint_threshold', 1.0)  # meters - "reached" distance
        self.declare_parameter('spiral_turns', 3)  # number of complete loops

        # Service clients
        self.goto_client = self.create_client(Trigger, '/vehicle/goto_neu')

        # Parameter client to set parameters on vehicle_interface node
        ...
        qos_profile = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,  # ← CRITICAL
    ...
    def calculate_distance(self, north1, east1, north2, east2):
        """Calculate distance between two local NED points in meters"""
    ...
    def generate_spiral_waypoints(self):
        """
        Generate spiral waypoints in NED frame (North, East offsets from home)
        Returns list of (north, east, altitude, yaw) tuples
        """
        ...
        # Calculate total angle and angular step
        total_angle = turns * 2 * math.pi  # radians
        num_points = int((turns * 2 * math.pi * radius) / spacing)  # arc length / spacing

        self.get_logger().info(f'Generating {num_points} waypoints for {turns} spiral turns')

        min_radius = 2.0  # Start at 2m instead of center
        ...
        for i in range(num_points):
            ...
            # Convert polar to Cartesian (NED frame)
            north = r * math.cos(theta)
            east = r * math.sin(theta)

            # Calculate yaw to point tangent to spiral (direction of travel)
            # Tangent angle = spiral angle + 90 degrees
        ...
        # Add final waypoint at full radius
    ...
    def wait_for_waypoint(self, target_north, target_east):
        ...
        while time.time() - start_time < timeout:
            # Spin to receive pose updates
            rclpy.spin_once(self, timeout_sec=0.1)  # ← ADD THIS
    ...
    def execute(self):
        ...
        for i, (north, east, alt, yaw) in enumerate(waypoints):
            ...
            # Set goto parameters on vehicle_interface
