...
# in_air hysteresis thresholds (metres, local ENU altitude - height above the
# EKF origin, not AGL and not AMSL). Climbing above UP latches in_air True;
# dropping below DOWN latches it False; between the two, hold the previous
# value - avoids flicker right at the boundary.
...
class MavrosInterface:
    ...
    def __init__(self, node: Node):
        ...
        # Derived, not mirrored: there is no telemetry field for "is it
        # flying". Latched in odom_callback rather than in get_telemetry(),
        # which is called from two threads - the callback runs only on the
        # executor thread and fires exactly when the altitude changes.
        self.in_air = False

        # Link liveness. _last_state_time is None until the first /mavros/state
        # arrives, and is the LOCAL receive time thereafter - so staleness needs
        # no agreement with the FC's clock. connected is MAVROS' own "do I have
        # a heartbeat from the FC" flag; it stays fresh while MAVROS lives, but
        # goes stale along with everything else if MAVROS itself dies - which is
        # the case the age check covers. Neither signal is sufficient alone.
        self.connected = False
        self._last_state_time = None
        # Declared here rather than in vehicle_controller because
        # MavrosInterface is constructed before vc declares its parameters.
        self.node.declare_parameter('fc_state_timeout_s', 3.0)
        self._state_timeout_s = self.node.get_parameter('fc_state_timeout_s').value
    ...
    def _update_in_air(self):
        """
        Latch in_air from the local altitude, with hysteresis. Called from
        odom_callback, so it advances once per new altitude sample.
        Note the false branch is altitude-only: disarming at height leaves
        in_air true until the altitude reading actually comes down.
        """
        if self.armed and self.odom_up > IN_AIR_UP_THRESHOLD_M:
            self.in_air = True
        elif self.odom_up < IN_AIR_DOWN_THRESHOLD_M:
            self.in_air = False
    ...
    def _state_link_ok(self) -> bool:
        """
        Is /mavros/state live? One check covering every way it can fail:
          _last_state_time is None -> nothing ever received (startup)
          age > timeout            -> messages stopped (MAVROS died)
          not connected            -> MAVROS alive, but no heartbeat from the FC

        Age uses local receive time, so no clock agreement with the FC is
        needed. This is a pure read, so it is safe to call from get_telemetry()
        on either thread - unlike the in_air latch, which had to live in a
        callback.
        """
        if self._last_state_time is None:
            return False
        age_s = (self.node.get_clock().now() - self._last_state_time).nanoseconds * 1e-9
        return age_s <= self._state_timeout_s and self.connected
    ...
    def get_telemetry(self):
        ...
        With no live link this reports _blank_telemetry() instead of the cached
        values. Every attribute below is written only by its callback, so when
        messages stop they simply keep whatever arrived last - the snapshot
        would otherwise go on describing a vehicle nobody is talking to.
        Reverting to the startup values makes "link dead" and "not started yet"
        the same state rather than two special cases, which is what lets
        safety_monitor's existing mode=='' test resolve both to UNKNOWN.
        """
        if not self._state_link_ok():
        ...
        return {
            'state': {
                'armed': self.armed,
                'mode': self.mode,
                # Derived here, not reported by the FC - see _update_in_air.
                'in_air': self.in_air,
            },
            'home': {
                'lat': self.home_lat,
...
def _blank_telemetry():
    """
    What get_telemetry() reports when there is no live link: exactly the values
    the attributes hold before any message has arrived. That equivalence is the
    point - "link dead" and "not started yet" become one state instead of two,
    so a consumer needs no separate check for the second.

    mode='' is the discriminator: no flight controller ever reports an empty
    mode string, so it is the one field distinguishable from real data. Every
    other value here is legal - position at the origin, disarmed, flat battery,
    no GPS fix - and a consumer reading any of them WITHOUT checking mode will
    be misled. Notably percentage=0.0 would satisfy any future low-battery
    condition, and up=0.0 reads as "on the ground".

    Built fresh per call: the nested dicts and the channels list are mutable and
    must not be shared between callers.

    Keep in step with __init__ and with get_telemetry()'s key structure - a new
    telemetry field has to be added in all three places.
    """
    return {
        'state': {'armed': False, 'mode': '', 'in_air': False},
        'home': {'lat': 0.0, 'lon': 0.0, 'alt': 0.0},
        'global_position': {'lat': 0.0, 'lon': 0.0, 'alt': 0.0},
        'local_position': {'east': 0.0, 'north': 0.0, 'up': 0.0, 'yaw_deg': 0.0},
        'velocity_body': {'right': 0.0, 'forward': 0.0, 'up': 0.0, 'yaw_rate': 0.0},
        'velocity_body_odom': {'right': 0.0, 'forward': 0.0, 'up': 0.0, 'yaw_rate': 0.0},
        'velocity_gps': {'east': 0.0, 'north': 0.0, 'up': 0.0},
        'imu': {
            'yaw_deg': 0.0,
            'gyro_right': 0.0, 'gyro_forward': 0.0, 'gyro_up': 0.0,
            'accel_right': 0.0, 'accel_forward': 0.0, 'accel_up': 0.0,
        },
        'battery': {'voltage': 0.0, 'current': 0.0, 'percentage': 0.0},
        'gps_status': {'fix_type': 0, 'satellites_visible': 0, 'eph': 0, 'epv': 0},
        'heading_deg': 0.0,
        'vfr_hud': {
            'airspeed': 0.0, 'groundspeed': 0.0, 'heading_deg': 0.0,
            'throttle': 0.0, 'altitude': 0.0, 'climb': 0.0,
        },
        'rc': {'channels': [], 'rssi': 0},
        'statustext': {'text': '', 'severity': 0},
    }
