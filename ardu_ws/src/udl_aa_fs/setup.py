...
setup(
    ...
    data_files=[
        ('share/ament_index/resource_index/packages',
            ['resource/' + package_name]),
        ('share/' + package_name, ['package.xml']),
        (f'share/{package_name}/launch', ['launch/vehicle_controller.launch.py']),
        (f'share/{package_name}/config', ['config/vehicle_controller.yaml']),
    ],
    install_requires=['setuptools'],
    zip_safe=True,
    maintainer='Your Name',
    ...
    entry_points={
        'console_scripts': [
            'vehicle_controller = udl_aa_fs.vehicle_controller:main',
            # 'vehicle_interface = ardupilot_autonomy.vehicle_interface:main',
